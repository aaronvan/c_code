...
void enqueue(LISTITEM *item) {
    ...
    head.last = item;		    // put the item in the q at the end
    item->prev = temp;		    // link back to the old 'last' item
    item->next = (LISTITEM *)&head; // set the forward link of the new itenm
    temp->next = item;		    // set forward link of old 'last' item to new one
}
